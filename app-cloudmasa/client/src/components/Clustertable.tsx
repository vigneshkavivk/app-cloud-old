...
const ClusterTable: React.FC<ClusterTableProps> = ({ clusters, loading = false }) => {
  const { hasRole } = useAuth(); // 👈 Get role info
  const canManage = hasRole('devops'); // devops or admin
  ...
  const handleDelete = async (id: string) => {
    if (!canManage) return; // Extra safety
    try {
  ...
  const handleSaveEdit = async () => {
    if (!editData?._id || !canManage) return;
    try {
                  ...
                  >
                    ...
                    <div className="flex gap-2">
                      {canManage ? (
                        ...
                        <span className="w-full text-center text-gray-500 text-sm py-2">No edit access</span>
                      )}
                    </div>
                  </div>
                ))}
