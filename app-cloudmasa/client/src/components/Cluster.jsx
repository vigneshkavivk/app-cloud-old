...
import { PlusCircle, Cloud, ArrowLeftCircle, XCircle, Lock } from 'lucide-react';
...
const CreateClusterPage = ({ handleBackToMyClusters, openClusterFormInitially = false }) => {
  ...
  if (!canProvision || !canViewCredentials) {
    return (
      <div className="bg-[#1E2633] min-h-screen p-6 flex items-center justify-center">
        <div className="bg-[#2A4C83] p-8 rounded-xl text-center max-w-md">
          <Lock className="h-12 w-12 mx-auto text-red-500 mb-4" />
          <h3 className="text-xl font-bold text-red-400 mb-2">Access Denied</h3>
          <p className="text-gray-300">
            You need <span className="font-mono">Agent.Provision</span> and <span className="font-mono">Credentials.View</span> to manage clusters.
          </p>
        </div>
      </div>
    );
...
const MyClusters = () => {
  const { hasPermission } = useAuth();
  const canViewClusters = hasPermission('Agent', 'View') || hasPermission('Agent', 'Provision'); // Adjust as per your RBAC model
        ...
        <>
          ...
          {showAddClusterPopup && (
            <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-[#2A4C83] p-8 rounded-xl shadow-2xl w-full max-w-md z-50 text-white">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-lg font-semibold text-white">Add or Create Cluster</h2>
                <button
              ...
              <p className="text-gray-400 mb-4">Select how you’d like to proceed:</p>
              <div className="flex gap-4">
                <div
                  onClick={handleAddExistingClusterClick}
                  className="border-2 border-gray-600 rounded-lg p-5 flex-1 text-center cursor-pointer hover:shadow-lg transition bg-[#1E2633]"
                >
                  <PlusCircle className="text-green-500 mx-auto mb-2" size={32} />
                  <p className="text-white">Add Existing Cluster</p>
                </div>
                <div
                  onClick={handleCreateClusterClick}
                  className="border-2 border-gray-600 rounded-lg p-5 flex-1 text-center cursor-pointer hover:shadow-lg transition bg-[#1E2633]"
                >
                  <Cloud className="text-blue-500 mx-auto mb-2" size={32} />
                  <p className="text-white">Create New Cluster</p>
                </div>
              </div>
            </div>
          )}
