...
const Login = () => {
  ...
  const handleOAuthLogin = (provider) => {
    window.location.href = `http://localhost:3000/auth/${provider}?redirect_uri=${window.location.origin}/oauth/callback`;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://app.cloudmasa.com:3000/api/auth/login', {
        method: 'POST',
      ...
      localStorage.setItem('user', JSON.stringify({
        ...
        role: data.user.role, // 👈 CRITICAL: Include role
        token: data.token
      }));
